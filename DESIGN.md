...
| 6 | Qwen2.5 via Ollama | **Any OpenAI-compatible endpoint**, defaulting to OpenRouter's free tier | Native tool-calling made the provider interchangeable. This paid off sooner than expected: GitHub Models, the original default, was retired on 30 July 2026, and moving off it cost three env vars and no code. The one hard requirement is native tool calling — the loop has no text-parsing fallback. |
